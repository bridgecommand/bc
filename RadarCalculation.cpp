...
void RadarCalculation::scan(irr::core::vector3d<int64_t> offsetPosition, const Terrain& terrain, const OwnShip& ownShip, const Buoys& buoys, const OtherShips& otherShips, irr::f32 weather, irr::f32 rain, irr::f32 tideHeight, irr::f32 deltaTime, uint64_t absoluteTime)
{
    ...
    for(irr::u32 i = 0; i<scansPerLoop;i++) { //Start of repeatable scan section
        ...
        for (irr::u32 currentStep = 1; currentStep<rangeResolution; currentStep++) { //Note that currentStep starts as 1, not 0. This is used in anti-rain clutter filter, which checks element at currentStep-1
            ...
            //Add land scan
            irr::f32 radarHeight = terrain.getHeight(localX,localZ) - dropWithCurvature - radarScannerHeight - tideHeight;
        ...
        if (!arpaOn) {
            ...
            if (arpaContacts.at(i).scans.size() > 1) {
                ...
                if ( absoluteTime - arpaContacts.at(i).scans.back().timeStamp > 60) {
                    ...
                    irr::s32 referenceScanIndex = currentScanIndex - 6;
...
// Try with f32 as inputs so we can do interpolation based on the theoretical start and end
void RadarCalculation::drawLine(irr::video::IImage* radarImage, irr::f32 startX, irr::f32 startY, irr::f32 endX, irr::f32 endY, irr::u32 alpha, irr::u32 red, irr::u32 green, irr::u32 blue)
{
...
void RadarCalculation::drawCircle(irr::video::IImage * radarImage, irr::f32 centreX, irr::f32 centreY, irr::f32 radius, irr::u32 alpha, irr::u32 red, irr::u32 green, irr::u32 blue)//Try with f32 as inputs so we can do interpolation based on the theoretical start and end
{
    ...
    irr::u32 radiusSquared = radarRadiusPx * radarRadiusPx;
    ...
    if (circumference > 0) {
        for (irr::f32 i = 0; i<=1; i += 1/circumference) {
            ...
            irr::s32 centreToX = thisX - radarRadiusPx;
            irr::s32 centreToY = thisY - radarRadiusPx;
            if (centreToX*centreToX + centreToY*centreToY <= radiusSquared) {
                if (thisX >= 0 && thisY >= 0) {
                    radarImage->setPixel(thisX,thisY,irr::video::SColor(alpha,red,green,blue));
                }
            }
...
irr::f32 RadarCalculation::rangeAtAngle(irr::f32 checkAngle,irr::f32 centreX, irr::f32 centreZ, irr::f32 heading)
{
    ...
    const irr::f32 distanceSqr = lambda * lambda + lambda*(2*centreX*sin(irr::core::DEGTORAD*heading) + 2*centreZ*cos(irr::core::DEGTORAD*heading)) + (std::pow(centreX,2) + std::pow(centreZ,2));
    if (distanceSqr > 0) {
