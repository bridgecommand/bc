...
HeadingIndicator::HeadingIndicator(irr::gui::IGUIEnvironment* environment, irr::gui::IGUIElement* parent, irr::core::rect<irr::s32> rectangle)
    : irr::gui::IGUIElement(irr::gui::EGUIET_ELEMENT,environment,parent,0,rectangle), heading(0)
{
}
...
void HeadingIndicator::draw()
{
    ...
    for (int ticAng = minAngle; ticAng<= maxAngle; ticAng++) {
        ...
        if (ticAng % 10 == 0) {
            ...
            font->draw(irr::core::stringw(displayAng),irr::core::rect<irr::s32>(startPoint.X - 100, AbsoluteRect.UpperLeftCorner.Y, startPoint.X + 100, AbsoluteRect.LowerRightCorner.Y),irr::video::SColor(skinAlpha,0,0,0),true,false,&AbsoluteRect);
        ...
        Environment->getVideoDriver()->draw2DLine(startPoint,endPoint,irr::video::SColor(skinAlpha,0,0,0));
    }
