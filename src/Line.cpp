...
void Line::update(irr::f32 deltaTime) // Calculate the force and torque acting on the ownship in the local coordinate system
{
  ...
  if (lineStart && lineEnd) {
    ...
    pModel->getOwnShip()->getSceneNode()->updateAbsolutePosition();
	...
	forceMagnitude = lineExtension * lineStiffness * pModel->getModelParameters().lineStiffnessFactor;

	// Add damping (50% of critical) here
	if (deltaTime > 0 && shipNominalMass > 0 && lineStiffness > 0) {
	  if ((pModel->getModelParameters().mode != OperatingMode::Multiplayer) || 
	    ...
	    forceMagnitude += lineExtensionSpeed * 0.5 * criticalDamping * pModel->getModelParameters().lineDampingFactor;
	  }
	}
    ...
    irr::core::matrix4 worldToLocal = pModel->getOwnShip()->getSceneNode()->getAbsoluteTransformation();
