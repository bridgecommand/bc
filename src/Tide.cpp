...
void Tide::setStreamOverrideDirection(irr::f32 aStreamDirection) //Range 0-360.
{
  mStreamOverrideDirection = aStreamDirection;
}

irr::f32 Tide::getStreamOverrideDirection() const
{
  return mStreamOverrideDirection;
}

void Tide::setStreamOverrideSpeed(irr::f32 aStreamSpeed) //Nm/h
{
  mStreamOverrideSpeed = aStreamSpeed;
} 

irr::f32 Tide::getStreamOverrideSpeed() const
{
  return mStreamOverrideSpeed;
}

void Tide::setStreamOverride(bool aStreamOverride)
{
  mIsStreamOverride = aStreamOverride;
}

bool Tide::getStreamOverride() const
{
  return mIsStreamOverride;
}

irr::core::vector2df Tide::getTidalStream(irr::f32 longitude, irr::f32 latitude, uint64_t requestTime) const
{
  if(mIsStreamOverride)
    {
      irr::core::vector2df overrideStream;
      overrideStream.X = sin(mStreamOverrideDirection*irr::core::DEGTORAD)*mStreamOverrideSpeed*KTS_TO_MPS;
      overrideStream.Y = cos(mStreamOverrideDirection*irr::core::DEGTORAD)*mStreamOverrideSpeed*KTS_TO_MPS;
