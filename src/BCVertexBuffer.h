...
namespace scene
{
	...
	class BCVertexBuffer : public IVertexBuffer
	{
		class IVertexList
		{
		public:
	...
	public:
		...
		BCVertexBuffer(video::E_VERTEX_TYPE vertexType) : Vertices(0),
				MappingHint(EHM_NEVER), ChangedID(1)
		{
			BCVertexBuffer::setType(vertexType);
		}

		BCVertexBuffer(const IVertexBuffer &VertexBufferCopy) :
				Vertices(0), MappingHint(EHM_NEVER),
				ChangedID(1)
		{
			BCVertexBuffer::setType(VertexBufferCopy.getType());
			BCVertexBuffer::reallocate(VertexBufferCopy.size());

			for (u32 n=0;n<VertexBufferCopy.size();++n)
				BCVertexBuffer::push_back(VertexBufferCopy[n]);
		}

		virtual ~BCVertexBuffer()
		{
			delete Vertices;
		}
