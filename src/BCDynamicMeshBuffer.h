...
namespace scene
{
	...
	class BCDynamicMeshBuffer: public IDynamicMeshBuffer
	{
	public:
		//! constructor
		BCDynamicMeshBuffer(video::E_VERTEX_TYPE vertexType, video::E_INDEX_TYPE indexType)
		: PrimitiveType(EPT_TRIANGLES)
		{
			VertexBuffer=new BCVertexBuffer(vertexType);
			IndexBuffer=new BCIndexBuffer(indexType);
		}

		//! destructor
		virtual ~BCDynamicMeshBuffer()
		{
			if (VertexBuffer)
				VertexBuffer->drop();
			if (IndexBuffer)
		...
		{
			BCDynamicMeshBuffer* clone = new BCDynamicMeshBuffer(VertexBuffer->getType(), IndexBuffer->getType());
	...
	private:
		BCDynamicMeshBuffer(const BCDynamicMeshBuffer&); // = delete in c++11, prevent copying
