...
private:
    ...
    uint64_t highTideTime(uint64_t startSearchTime, int searchDirection=1) const; //Find previous or next high tide time
    uint64_t lowTideTime(uint64_t startSearchTime, int searchDirection=1) const; //Find previous or next low tide time
